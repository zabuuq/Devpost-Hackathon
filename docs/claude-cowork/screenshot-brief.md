...
| 07 | `07_probe_revealed.png` | Step 11 | Target Grid showing the result of the probe firing: a 7 by 7 blue illumination overlay around `(70, 11)` with the cells of the Player 2 Battleship that fall inside the probe area now visible. The whole Battleship lands inside the 7 by 7 box for this shot, so it reads as a complete ship; under partial overlap only the in-box cells render. Battle Log on the left has a new probe entry. | Cursor outside the highlight |
...
- **Partial probe reveal (I9-1):** Probes reveal only the cells of an enemy ship that fall inside the probe area. Cells of the same ship outside the probe area stay un-rendered until a future probe overlaps them or a hit lands. The Section 4 path's probe is sized to encompass the whole Battleship by design, so shots 06–11 read as full-ship reveals; partial reveals show only the overlapping cells.
