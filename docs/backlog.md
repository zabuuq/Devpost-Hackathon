...
| Use graphics for the interface | The UI today is built almost entirely from default Godot Controls (Button, Label, HSlider, Panel) with text-only headers and tinted modulates. Replace with custom panel art / button frames / themed widgets to match the nebula aesthetic. Touchpoints: `scripts/ui/ship_panel.gd` (accordion rows), `scripts/main_menu.gd`, `scripts/handoff.gd`, `scripts/victory.gd`, plus a project-wide Theme resource. Probably wants per-ship portrait art too. |
