...
| Ship visibility around active probes is buggy | Ships entering, leaving, or moving within an active probe area on the Target Grid still produce wrong fog state in some cases. Will be addressed as part of a larger probe-mechanic rework in a future iteration. Tracking this row as the umbrella for that rework so smaller related fixes don't ship piecemeal. |
