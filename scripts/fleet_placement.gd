...
func _setup_ship_list() -> void:
	for i in range(ShipDefinitions.FLEET.size()):
		var ship_type: String = ShipDefinitions.FLEET[i]
		var btn := Button.new()
		var squares: int = ShipDefinitions.SHIPS[ship_type]["squares"]
		btn.text = "%s (%d sq)" % [SHIP_NAMES.get(ship_type, ship_type), squares]
...
func _update_detail_panel(ship_type: String) -> void:
	var stats: Dictionary = ShipDefinitions.SHIPS[ship_type]
	detail_name.text = SHIP_NAMES.get(ship_type, ship_type)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_Q:
			ghost_facing = (ghost_facing - 1 + 4) % 4
			grid_node.queue_redraw()
		elif event.keycode == KEY_E:
			ghost_facing = (ghost_facing + 1) % 4
			grid_node.queue_redraw()
...
func _screen_to_grid(local_pos: Vector2) -> Vector2i:
	...
	var scale_v := sub_vp_size / vp_size
	var world_pos := local_pos * scale_v
...
func _is_placement_valid(ship_type: String, origin: Vector2i, facing: int) -> bool:
	var cells := ShipDefinitions.get_ship_cells(ship_type, origin, facing)
	for cell in cells:
...
func _draw_grid() -> void:
	...
	for idx in placed_ships:
		...
		if cells.size() > 0:
			var fc := cells[0]
			var center := Vector2(fc.x * CELL_SIZE + CELL_SIZE / 2.0, fc.y * CELL_SIZE + CELL_SIZE / 2.0)
			grid_node.draw_circle(center, 4.0, Color(1.0, 1.0, 0.3, 1.0))
	if selected_ship_idx >= 0:
		var ship_type: String = ShipDefinitions.FLEET[selected_ship_idx]
		ghost_valid = _is_placement_valid(ship_type, ghost_position, ghost_facing)
		var ghost_color := Color(0.3, 1.0, 0.3, 0.5) if ghost_valid else Color(1.0, 0.2, 0.2, 0.5)
		var cells := ShipDefinitions.get_ship_cells(ship_type, ghost_position, ghost_facing)
		for cell in cells:
